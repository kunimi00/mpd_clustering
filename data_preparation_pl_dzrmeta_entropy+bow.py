...
# (2) entropy of feature distributions
...
for _idx in tqdm(range(len(all_pl_with_dzrmeta))):
    ...
    # 2-1) BOW Cnt vector
    _curr_pl_feature_cnt_vector = []
    for _tidx in range(len(track_features_to_investigate)):
        tr_feature = track_features_to_investigate[_tidx]
        # print(_curr_pl_dict_for_feature_counts[tr_feature].shape)
        _sorted_vec = - np.sort(- _curr_pl_dict_for_feature_counts[tr_feature])
        if np.sum(_sorted_vec) > 0:
            _sorted_vec /= np.sum(_sorted_vec)

        _curr_pl_feature_cnt_vector.extend(_sorted_vec)

    _pl_feature.extend(_curr_pl_feature_cnt_vector)
